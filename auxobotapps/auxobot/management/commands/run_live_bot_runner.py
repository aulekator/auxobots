...
class Command(BaseCommand):
    help = 'Runs live trading bots for all active users (main systemd entry point)'

    def handle(self, *args, **options):
        self.stdout.write("Auxobot Live Bot Runner started — monitoring active users...")

        processes = {}

        while True:
            try:
                active_configs = BotConfig.objects.filter(is_active=True)
                ...
                for config in active_configs:
                    ...
                    if user_id not in processes or not processes[user_id].is_alive():
                        ...
                        env = os.environ.copy()
                        env['USER_SESSION_ID'] = str(user_id)
